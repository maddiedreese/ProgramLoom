import { ArrowLeft, CalendarClock, CheckCircle2, ChevronRight, ExternalLink, FileInput, GitBranch, GripVertical, Layers3, LoaderCircle, Plus, Send, Settings2, Tag, Trash2, UsersRound } from "lucide-react";
import { type FormEvent, useEffect, useState } from "react";
import { useParams } from "react-router-dom";

type User = { id: string; email: string; name: string };
type EventRecord = { id: string; organizationName: string; organizationSlug: string; name: string; slug: string; timezone: string; startsAt: string; endsAt: string; status: string; storageMode: string };
  ...
  return <div className="event-workspace">
    <aside className="event-sidebar">
      ...
      <nav className="event-nav" aria-label="Event workspace">
        ...
        <a href={`/app/events/${eventId}/submissions`}><UsersRound size={18} /> Submissions</a><a href={`/app/events/${eventId}/reviews`}><CheckCircle2 size={18} /> Reviews</a><a href={`/app/events/${eventId}/speakers`}><UsersRound size={18} /> Speakers</a><a href={`/app/events/${eventId}/agenda`}><CalendarClock size={18} /> Agenda</a>
      </nav>
      ...
      <div className="cfp-layout">
        ...
        <section className="builder-panel">
          {!selected ? <div className="builder-empty"><FileInput size={35} /><h2>Create your first CFP</h2><p>Give it a name, then assemble the questions your program team needs.</p></div> : <>
            <div className="builder-toolbar"><div><span className={`status-dot ${selected.publishedAt ? "live" : ""}`} />{selected.publishedAt ? "Published" : "Draft"}<strong>{selected.name}</strong>{selected.publishedAt && event && <a className="public-form-link" href={`/c/${event.organizationSlug}/${event.slug}/${selected.slug}`} target="_blank" rel="noreferrer">Open public form <ExternalLink size={12} /></a>}</div><div className="builder-tabs"><button className={panel === "fields" ? "active" : ""} onClick={() => setPanel("fields")}><Layers3 size={15} /> Fields</button><button className={panel === "logic" ? "active" : ""} onClick={() => setPanel("logic")}><GitBranch size={15} /> Logic</button><button className={panel === "settings" ? "active" : ""} onClick={() => setPanel("settings")}><Settings2 size={15} /> Settings</button></div></div>
