...
- The repeatable local D1 smoke protocol passed create/import/search → note/custom field → dynamic segment → pipeline move/note/history → event handoff → public interest-form publication → duplicate merge with duplicate-email selection → live dashboard totals on 2026-08-09. Unit and authorization-boundary coverage brings the project suite to 19 passing tests.
