...
type DeliverablesReminderMessage = {
  email: string;
  name: string;
  eventName: string;
  portalLink: string;
...
export async function sendMagicLink(
  ...
  const action =
    message.purpose === "register"
      ? "create your ProgramLoom account"
      : "sign in to ProgramLoom";
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      subject:
        message.purpose === "register"
          ? "Your ProgramLoom account link"
          : "Your ProgramLoom sign-in link",
...
export async function sendInvitation(
  ...
  if (!env.RESEND_API_KEY || !env.EMAIL_FROM)
    throw new Error("Transactional email is not configured.");
  const scope = message.eventName
    ? `${message.eventName} in ${message.organizationName}`
...
export async function sendSubmissionConfirmation(
  env: Env,
  message: SubmissionConfirmationMessage,
): Promise<string> {
  if (!env.RESEND_API_KEY || !env.EMAIL_FROM)
    throw new Error("Transactional email is not configured.");
  const subject =
    message.subject || `We received your proposal for ${message.eventName}`;
  const intro =
    message.body ||
    `Thanks for sharing your idea with the ${message.eventName} program team.`;
  const response = await fetch("https://api.resend.com/emails", {
    method: "POST",
    headers: {
      authorization: `Bearer ${env.RESEND_API_KEY}`,
      "content-type": "application/json",
      "idempotency-key": `submission/${message.idempotencyKey}`,
    },
    body: JSON.stringify({
      from: env.EMAIL_FROM,
      to: [message.email],
...
export async function sendDecision(
  ...
  if (!env.RESEND_API_KEY || !env.EMAIL_FROM)
    throw new Error("Transactional email is not configured.");
  const action = message.portalLink
    ? `<p style="margin:28px 0"><a href="${escapeHtml(message.portalLink)}" style="display:inline-block;background:#315c45;color:white;text-decoration:none;padding:14px 18px;border-radius:8px;font-weight:700">Open speaker portal</a></p>`
    : "";
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    headers: {
      authorization: `Bearer ${env.RESEND_API_KEY}`,
      "content-type": "application/json",
      "idempotency-key": `decision/${message.idempotencyKey}`,
    },
    body: JSON.stringify({
      from: env.EMAIL_FROM,
      to: [message.email],
      ...
      html: `<!doctype html><html><body style="margin:0;background:#f4f1e8;color:#20241f;font-family:Arial,sans-serif"><div style="max-width:560px;margin:0 auto;padding:40px 24px"><div style="font-size:20px;font-weight:700;margin-bottom:34px">ProgramLoom</div><div style="background:#fffdf6;border:1px solid #d9d5ca;border-radius:14px;padding:32px"><p>Hi ${escapeHtml(message.name)},</p>${message.body
        .split("\n")
        .filter(Boolean)
        .map((paragraph) => `<p>${escapeHtml(paragraph)}</p>`)
        .join(
          "",
        )}${action}<p style="font-size:13px;color:#63675f">This message was sent by the ${escapeHtml(message.eventName)} program team about “${escapeHtml(message.sessionTitle)}”.</p></div></div></body></html>`,
...
export async function sendCrmOutreach(
  env: Env,
  message: CrmOutreachMessage,
): Promise<string> {
  if (env.APP_ENV === "test")
    return `test-${message.campaignId}-${message.email}`;
  if (!env.RESEND_API_KEY || !env.EMAIL_FROM)
    throw new Error("Transactional email is not configured.");
  const response = await fetch("https://api.resend.com/emails", {
    method: "POST",
    headers: {
      authorization: `Bearer ${env.RESEND_API_KEY}`,
      "content-type": "application/json",
      "idempotency-key": `crm/${message.campaignId}/${await stableMessageKey(message.email, message.subject)}`,
    },
    body: JSON.stringify({
      from: env.EMAIL_FROM,
      to: [message.email],
      ...
      html: `<!doctype html><html><body style="margin:0;background:#f4f1e8;color:#20241f;font-family:Arial,sans-serif"><div style="max-width:560px;margin:0 auto;padding:40px 24px"><div style="font-size:20px;font-weight:700;margin-bottom:34px">ProgramLoom</div><div style="background:#fffdf6;border:1px solid #d9d5ca;border-radius:14px;padding:32px"><p>Hi ${escapeHtml(message.name)},</p>${message.body
        ...
        .join(
          "",
        )}<p style="font-size:13px;color:#63675f">Sent by an event team using ProgramLoom.</p></div></div></body></html>`,
      text: `Hi ${message.name},\n\n${message.body}\n\nSent by an event team using ProgramLoom.`,
      tags: [{ name: "message_type", value: "crm_outreach" }],
    }),
  });
  if (!response.ok) {
    const detail = await response.text();
...
export async function sendDeliverablesReminder(
  env: Env,
  message: DeliverablesReminderMessage,
): Promise<string> {
  if (env.APP_ENV === "test") return `test-${message.batchId}-${message.email}`;
  if (!env.RESEND_API_KEY || !env.EMAIL_FROM)
    throw new Error("Transactional email is not configured.");
  const items = message.outstanding
    .map(
      (task) =>
        `<li><strong>${escapeHtml(task.title)}</strong>${task.dueAt ? ` — due ${escapeHtml(new Date(task.dueAt).toLocaleDateString("en-US", { dateStyle: "medium", timeZone: "UTC" }))}` : ""}</li>`,
    )
    .join("");
  const textItems = message.outstanding
    .map(
      (task) =>
        `- ${task.title}${task.dueAt ? ` — due ${new Date(task.dueAt).toLocaleDateString("en-US", { dateStyle: "medium", timeZone: "UTC" })}` : ""}`,
    )
    .join("\n");
  const response = await fetch("https://api.resend.com/emails", {
    method: "POST",
    headers: {
      authorization: `Bearer ${env.RESEND_API_KEY}`,
      "content-type": "application/json",
      "idempotency-key": `deliverables/${message.batchId}/${await stableMessageKey(message.email, message.portalLink)}`,
    },
    body: JSON.stringify({
      from: env.EMAIL_FROM,
      to: [message.email],
      reply_to: env.EMAIL_REPLY_TO,
      subject: `${message.eventName}: outstanding speaker deliverables`,
      html: `<!doctype html><html><body style="margin:0;background:#f4f1e8;color:#20241f;font-family:Arial,sans-serif"><div style="max-width:560px;margin:0 auto;padding:40px 24px"><div style="font-size:20px;font-weight:700;margin-bottom:34px">ProgramLoom</div><div style="background:#fffdf6;border:1px solid #d9d5ca;border-radius:14px;padding:32px"><p>Hi ${escapeHtml(message.name)},</p><p>The ${escapeHtml(message.eventName)} team is waiting on:</p><ul>${items}</ul><p style="margin:28px 0"><a href="${escapeHtml(message.portalLink)}" style="display:inline-block;background:#315c45;color:white;text-decoration:none;padding:14px 18px;border-radius:8px;font-weight:700">Open speaker portal</a></p></div></div></body></html>`,
      text: `Hi ${message.name},\n\nThe ${message.eventName} team is waiting on:\n${textItems}\n\nOpen your speaker portal: ${message.portalLink}`,
      tags: [{ name: "message_type", value: "deliverables_reminder" }],
    }),
  });
  if (!response.ok) {
    const detail = await response.text();
...
async function stableMessageKey(email: string, link: string): Promise<string> {
  const digest = await crypto.subtle.digest(
    "SHA-256",
    new TextEncoder().encode(`${email}|${link}`),
  );
...
function renderInvitationHtml(message: InvitationMessage): string {
  const scope = message.eventName
    ? `${escapeHtml(message.eventName)} in ${escapeHtml(message.organizationName)}`
