...
const acceptInvitationSchema = invitationTokenSchema.extend({
  name: z.string().trim().min(2).max(120).optional(),
});
...
router.post("/request", zValidator("json", requestSchema), async (context) => {
  ...
  const existing = await db
    .prepare("SELECT id, name FROM users WHERE email = ? COLLATE NOCASE")
    .bind(input.email)
  ...
  if (input.mode === "login" && !existing)
    return context.json({
      ok: true,
      message: "If an account exists, a secure link is on its way.",
    });
  if (input.mode === "register" && !input.name)
    return context.json(
      { error: { code: "name_required", message: "Enter your full name." } },
      400,
    );
  ...
  await db.batch([
    db
      .prepare(
        "DELETE FROM auth_challenges WHERE email = ? COLLATE NOCASE AND purpose = ? AND consumed_at IS NULL",
      )
      .bind(input.email, purpose),
    db
      .prepare(
        "INSERT INTO auth_challenges (id, email, purpose, token_hash, redirect_path, expires_at, metadata_json) VALUES (?, ?, ?, ?, ?, ?, ?)",
      )
      .bind(
        challengeId,
        input.email,
  ...
  try {
    await sendMagicLink(context.env, {
      email: input.email,
  ...
  return context.json({
    ok: true,
    message: "Check your inbox for a secure link. It expires in 15 minutes.",
  });
});
...
router.get("/verify", async (context) => {
  ...
  if (!rawToken)
    return context.redirect(`${context.env.APP_URL}/login?error=invalid_link`);
  const tokenHash = await sha256(rawToken);
  ...
  let user = await db
    .prepare("SELECT id, email, name FROM users WHERE email = ? COLLATE NOCASE")
  ...
  if (!user && challenge.purpose === "register") {
    ...
    await db
      ...
      .bind(
        id,
        challenge.email,
        metadata.name ?? challenge.email.split("@")[0],
    ...
    user = {
      ...
      name: metadata.name ?? challenge.email.split("@")[0],
    };
  }
  if (!user)
    return context.redirect(`${context.env.APP_URL}/login?error=invalid_link`);
  ...
  await db.batch([
    ...
    db
      ...
      .bind(sessionId, user.id, sessionHash, sessionExpires.toISOString()),
  ]);
...
router.post(
  "/invitations/preview",
  zValidator("json", invitationTokenSchema),
  async (context) => {
    const db = requireDatabase(context.env);
...
router.post(
  ...
  async (context) => {
    ...
    let user = await db
      .prepare(
        "SELECT id, email, name FROM users WHERE email = ? COLLATE NOCASE",
      )
      .bind(invitation.email)
      .first<{ id: string; email: string; name: string }>();
    if (!user && !name)
      return context.json(
        {
          error: {
            code: "name_required",
            message: "Enter your full name to accept this invitation.",
          },
        },
        400,
      );
    ...
    statements.push(
      db
        .prepare(
          "INSERT INTO organization_members (organization_id, user_id, role) VALUES (?, ?, ?) ON CONFLICT (organization_id, user_id) DO NOTHING",
        )
        .bind(invitation.organizationId, user.id, organizationRole),
    );
    if (invitation.eventId)
      statements.push(
        db
          .prepare(
            "INSERT INTO event_members (event_id, user_id, role, invited_by) VALUES (?, ?, ?, ?) ON CONFLICT (event_id, user_id, role) DO NOTHING",
          )
          .bind(
            invitation.eventId,
            user.id,
    ...
    if (invitation.role === "speaker" && invitation.eventId)
      statements.push(
        eventManagerNotificationStatement(db, {
          organizationId: invitation.organizationId,
          eventId: invitation.eventId,
          category: "speaker",
          notificationType: "speaker.invitation_accepted",
          severity: "info",
          title: "A speaker accepted their portal invitation",
          body: user.name,
          actionUrl: `/app/events/${invitation.eventId}/speakers`,
          entityType: "invitation",
          entityId: invitation.id,
          coalesceKey: `speaker-invitation-accepted:${invitation.id}`,
        }),
      );
